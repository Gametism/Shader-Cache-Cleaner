...
if %ERRORLEVEL% == 2 (
    echo Deletion of graphics driver caches canceled.
    echo Graphics driver cache deletion skipped. >> "%LogFile%"
) else (
    echo Starting cache cleaning process...
    echo. >> "%LogFile%"
    :: Delete graphics driver shader caches
...
:: Ask if user wants to add a custom path to delete, even if they chose not to delete the shader caches
set "CustomGameCache="
